...
def main():
    ...
    while True:
        ...
        ether = Ether()
        total_balance = 0  # Variabel untuk menyimpan total balance semua user
        
        for index, query in enumerate(queries, start=1):
            print_(f"SxG========= Account {index}/{sum_queries} =========SxG")
            token = ether.get_token(query)
            if token is not None:
                user_info = ether.get_user_info(token)
                balance = user_info.get('balance', 0)  # Ambil balance user
                total_balance += balance  # Tambahkan balance ke total_balance
                print_(f"TGID : {user_info.get('tgId','')} | Username : {user_info.get('tgUsername','None')} | Balance : {balance}")
                ...
                if data_order is not None:
                    ...
                    for list_ in list_periods:
                        period = list_.get('period', {})
                        unlockThreshold = period.get('unlockThreshold', 0)
                        detail_order = list_.get('order', {})
                        id = period.get('id', 1)
                        
                        if detail_order is not None:
                            statusss = detail_order.get('status', '')
                            if statusss == "CLAIM_AVAILABLE":
                                ether.claim_order(token=token, order=detail_order)
                            elif statusss == "NOT_WIN":
                                ether.mark_checked(token=token, order=detail_order)
                        
                        if totalScore >= unlockThreshold and detail_order is None:
                            status = [True, False]
                            coins = random.choice(detail_coin) if input_coin == 'y' else detail_coin[0]
                            status_order = (False if input_order == 'l' else True) if input_order in ['l', 's'] else random.choice(status)
        ...
        print_("\n")
        print_(f"Total Balance of All Users: {total_balance}")
